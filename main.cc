...
void* calcMaxFlops(void* data)
{
  ...
  ne10_vec4f_t a; ne10_setc_vec4f(&a, &one, 1);
  ne10_vec4f_t b; ne10_setc_vec4f(&b, &one, 1);
  ne10_vec4f_t c; ne10_setc_vec4f(&c, &one, 1);
  ne10_vec4f_t d; ne10_setc_vec4f(&d, &one, 1);
  ne10_vec4f_t e; ne10_setc_vec4f(&e, &one, 1);
  ne10_vec4f_t f; ne10_setc_vec4f(&f, &one, 1);
  ne10_vec4f_t g; ne10_setc_vec4f(&g, &one, 1);
  ne10_vec4f_t h; ne10_setc_vec4f(&h, &one, 1);
  ne10_vec4f_t i; ne10_setc_vec4f(&i, &zero, 1);
  ne10_vec4f_t j; ne10_setc_vec4f(&j, &zero, 1);
  ne10_vec4f_t k; ne10_setc_vec4f(&k, &zero, 1);
  ne10_vec4f_t l; ne10_setc_vec4f(&l, &zero, 1);
  ne10_vec4f_t m; ne10_setc_vec4f(&m, &zero, 1);
  ne10_vec4f_t n; ne10_setc_vec4f(&n, &zero, 1);
  ne10_vec4f_t o; ne10_setc_vec4f(&o, &zero, 1);
  ne10_vec4f_t p; ne10_setc_vec4f(&p, &zero, 1);
  
  volatile ne10_vec4f_t result;
...
int main()
{
  std::cout << "Starting ARM-correlator... Modified by Mitchell Cox." << std::endl;
  ...
  cout << "total maxFlops with " << nrThreads << " threads is: " << maxFlops << std::endl;
	...
	p[t].version = CORRELATOR_REFERENCE; // CHANGE THIS TO USE ANOTHER VERSION
